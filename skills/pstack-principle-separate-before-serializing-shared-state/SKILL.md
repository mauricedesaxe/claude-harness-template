...
**Repository writes:** When multiple agents write in the same repository on one machine or sandbox, each agent gets its own jj workspace. If the git repository has no `.jj`, run `jj git init --colocate` before fan-out. Do not use git worktrees as the parallel isolation unit after colocation. Each jj workspace owns a separate working-copy commit, while all workspaces share the repository graph.
