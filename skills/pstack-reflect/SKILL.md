...
Resolve each role from `models.md`, overridden by `~/.lazar-harness/models.md` when present. Where the `pool` holds more than one family, put the two judgment lenses on different `pool` entries so the divergent lens gets fresh eyes.

| Lens | role | Prompt template |
|---|---|---|
| Judgment | `judge` | `references/judgment-reviewer.md` |
| Tooling | `code-hard` | `references/tooling-reviewer.md` |
| Divergent | `judge` (a different `pool` family from Judgment where one exists) | `references/divergent-reviewer.md` |

Pass each template verbatim, substituting the transcript path or digest where marked. Reviewers return findings in the `Task` response body.

### 3. Synthesize

One `Task` call, `subagent_type: general-purpose`, using the `judge` role, agent mode (`readonly: false`). The synthesizer's quality check includes spot-verifying citations, which can require MCP access; readonly strips MCPs. Use `references/synthesizer.md` verbatim, with each reviewer's full output inlined where marked. The synthesizer returns a structured Accepted / Rejected / Backlog list.
