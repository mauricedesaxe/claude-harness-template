...
Launch all reviewers in a single message using the Task tool. Use the `pool` from `models.md`, resolved through `~/.lazar-harness/models.md` when present, one reviewer per entry. The list length sets the reviewer count.

For each reviewer:
- `subagent_type`: `general-purpose`
- `model`: one entry from the `pool`
- `readonly`: `true`

The adversarial signal comes from `pool` diversity. When `pool` lists slugs from different vendors, that is where interrogate earns the most. When `pool` has one entry, run that model N times, keep the parallel coverage, and state that the diversity was thin so the verdict reads lower-confidence. If a `pool` slug is unresolvable when you try to spawn the subagent, drop that entry, proceed with the rest, and note the drop. Never block the review on one bad slug.
