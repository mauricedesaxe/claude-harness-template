...
Produce a phased implementation plan grounded in the **Principles** section of the `pstack-poteto-mode` skill. The plan is the deliverable. Do not implement.
...
Read the **Principles** section of the `pstack-poteto-mode` skill end to end, and the leaf `principle-*` skills it indexes. The principles govern every plan decision; cross-link them.
...
- Pass `model:` explicitly per the roles in `models.md` (`code` for implementation, `judge` for judgment).
...
- Browser / Electron / Web UIs: the **lazar-qa** skill.
- CLIs and TUIs: drive it yourself, or the `run` skill.
...
In the overview, name which pstack-poteto-mode non-negotiables the implementer must apply, by name:

- the **pstack-how** skill over each unfamiliar subsystem before changing it.
- the **pstack-interrogate** skill for adversarial review on contested designs before shipping.
- the **pstack-unslop** skill over each diff before commit and over any prose surface.
- the **pstack-show-me-your-work** skill to keep a decision trail when the plan is large enough to need an auditable record.
