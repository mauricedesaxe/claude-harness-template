...
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting works to depth 3, and a nested spawn has the full Task schema including its background and isolation options). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Run in the background, and as a real cloud agent under the background-agents runtime, unless the task needs this machine: browser or CLI runtime verification (the **lazar-qa** or `run` skill); reading local transcripts; simulators and local IDE state; auth that exists only here. A cloud agent cannot read the local store, so its brief inlines what it needs or points at repo paths. Prefer fewer, broader workers; one writer per jj workspace or bookmark (pstack-principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker, drawn from `pool`.
...
SCOPE        paths this unit may write; paths it may not; its exclusive jj workspace or bookmark
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` from jj and `gh` after every merge and stack mutation, because GitHub base refs drift mid-rebase while the jj bookmarks are authoritative: ordered PR list, bookmark names, head SHAs, a generation number, the lowest unmerged PR. Resolve it from the jj repo, which knows the stack; a checkout whose jj metadata never saw the pushes reports no PRs and the command errors rather than guessing.
- Exactly one stacker per stack may run `jj rebase`, serialized within its stack; record the holder in the standing orders. Rebases run on a dedicated worker; a rebase of a large stack on the main checkout collides with every other agent's `@` (`§28`).
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the cloud agent's status in your runtime's dashboard. Transcript mtime is not liveness.
