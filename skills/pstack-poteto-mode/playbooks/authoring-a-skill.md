...
1. Write the `SKILL.md`. Frontmatter needs a `name` and a `description`; the body is agent-facing prose held to this playbook's bar. A skill vendored from an upstream follows the harness vendoring pattern (`vendor-skills.sh`, `skills-lock.json`) rather than a hand copy.
