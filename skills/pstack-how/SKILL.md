...
- `model`: the `code` role from `models.md` (resolved through `~/.lazar-harness/models.md` when present)
- `readonly`: `true`
...
- `subagent_type`: `general-purpose`
- `model`: the `judge` role from `models.md`
- `readonly`: `true`
...
After the explanation is complete, spawn one architectural critic per entry in the `pool` from `models.md` (resolved through `~/.lazar-harness/models.md` when present), all in a single message.

For each critic:
- `subagent_type`: `general-purpose`
- `model`: one entry from the `pool`. These are minimum reasoning levels. The lead should escalate any model when the architecture warrants deeper analysis.
- `readonly`: `true`
