...
3. Pick the runners. Use the `pool` from `models.md`, resolved through `~/.lazar-harness/models.md` when present, one runner per entry. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
4. Assign output paths. Prepare one jj workspace per repository candidate through the **separate-before-serializing-shared-state** principle skill. A candidate that produces an independent artifact writes under `/tmp/arena-<slug>/candidate-<n>/`. N candidates writing to the same path is shared mutable state and fails that principle skill test.
