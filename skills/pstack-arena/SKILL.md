...
4. Assign output paths. Each candidate writes to its own location (an isolated working copy, a jj workspace or a git worktree in a non-jj repo, where possible, otherwise `/tmp/arena-<slug>/candidate-<n>/`). N candidates writing to the same path is shared mutable state and fails the **separate-before-serializing-shared-state** principle skill test.
