...
**Why.** TypeScript covers the full stack with one toolchain, one type system, and one
package-management discipline.

**Earn-its-keep.**

- **Python** earns its keep for three things. Notebook-driven analysis and
  exploratory data work. The ML ecosystem (PyTorch, scikit-learn, the
  HuggingFace stack). Scientific computing where the library you need is
  Python-native. *Not* for a
  web service where TypeScript would do.
- **Rust** earns its keep when the work is *measurably* memory- or CPU-bound.
  A parser on a hot path, a sandbox that isolates untrusted code, a CLI that
  ships as a single binary. It is rarely actually needed. Reach for it
...
where this is wrong are conspicuous when they arrive, so you don't have to
guess. They are horizontal scaling, geographic distribution, and a true fan-out
streaming workload.
...
- **Horizontal scaling + a load balancer** earns its keep when one vertically
  scaled instance saturates CPU at peak. Vertical scaling is cheap and
...
codebase because the modular cut held up. A module can genuinely earn the right to
become its own service per §1. The modular cut keeps that extraction to a
tractable diff instead of a rewrite.
...
- **SQLite**. Single-file tempts, but to operationalise backup, migration,
...
PostGIS, pgvector, and TimescaleDB-the-extension cover most "specialty" needs.
Most workloads never outgrow it. To pick *anything* else by default trades
durable simplicity for early-stage cleverness.
...
**Why.** The single biggest time sink in early-stage software is infrastructure
you didn't have to own. Managed platforms collapse "build, run,
...
upgrades, security patches, certificate rotation, log shipping, and cost
monitoring. It is rarely justified until you reach a scale where you actually
want a dedicated platform team.
...
once you count N+1s, transactions, and aggregations. To push the app to the
edge moves it *closer to the user* and *farther from the database*. That trades
the smaller round-trip down for the bigger one up, and you now pay cold-starts. The math almost never works for stateful, DB-backed apps.

Serverless adds cold starts and the connection-pool problem, because every cold
function wants its own pool. It has no in-process caches and no shared in-memory
state, so §10's primitives can't be in-process. Local development gets much
harder, observability gets harder, and you lock in to whatever runtime the
platform exposes.
...
A different CDN earns its keep in three cases. A specific geographic gap in
Cloudflare's PoP coverage matters for your users, which is rare. An org-policy
reason names a specific alternative. Or you measured a concrete deficiency.
...
- DataDog by default. The cost curve grows non-linearly and turns hostile at the
  scales where managed observability should be cheapest.
- Any observability primitive built in-house. Almost never the right call (§13).
...
rules**. Business logic in SQL is invisible to your code search and written in a
different language. It is harder to test, harder to evolve, and it silently
drifts out of sync with the app-side version. Keep it out.
...
- **`CHECK` constraints** that encode anything beyond simple, stable, invariant
  range/shape checks. `CHECK (price >= 0)` is fine, because that's a hard invariant.
  `CHECK (status IN ('draft', 'submitted', 'approved'))` is right at the
  line. It is defensible if the enum is genuinely stable. Push back on it if the
...
went unread for years) is fine. Document it. A trigger that protects a
...
Flags in your own Postgres also participate in §13's "own the data" sub-rule.
Flag history, exposure events, and who-toggled-what all live where the rest of
your data lives.
...
feature too big to ship atomically, an A/B cut, or a fast kill switch for a
...
non-trivial to operate. Connection pooling needs care and debugging is harder.
So it earns its keep when the failure mode, a data leak, is unacceptable. That
means commercial systems.
...
- **View state in the URL.** Filters, search, open modal: encode them in the URL
...
**Rule.** When a process is **lengthy or flaky**, run it in the background. The
user clicks and the API enqueues a job. The worker processes it. The user sees
the result via polling (§25) or on the next page load.

The user never waits on something slow, and never watches it fail in their face.
**Every job is idempotent**, so a re-run produces the same outcome. This is a
hard line.
...
Not in a third-party scheduler dashboard, and not in a Kubernetes CronJob YAML
the app doesn't own. Graphile Worker's cron support, a `node-cron` declaration next to
the worker, or an equivalent in-code declaration is the pattern.

**Idempotency is a hard line.** Every job is idempotent, so a re-run produces
the same outcome. This is non-negotiable, because retries are inevitable: network
blips, worker crashes mid-job, queue redelivers.

A job that can't be safely re-run is a bug waiting to be reported. The pattern: a job takes a target
identifier, an order ID or a user ID. It checks current state, and applies
changes only where the work is not already done. "Send the welcome email" is idempotent by
...
internal bugs and miss the wiring bugs.

**Earn-its-keep.** The synchronous request path earns its keep on two counts
together. (a) The work is fast and reliable. (b) The user genuinely needs the
result before the next action. Otherwise default to async, even for "fast" work that has
flake risk.
...
R2 specifically, for four reasons. It aligns with §9 Cloudflare. It has **no
egress fees**, the killer feature for media-heavy products. Its API is
S3-compatible, so libraries port without rewrites. The pricing is reasonable.
...
**Public assets** (CSS, JS bundles, static images that ship with the app) are
Cloudflare Pages handles these per §9, not R2. R2 is for *user* and *runtime*
bytes, not deploy artefacts.

**Earn-its-keep.** S3, DigitalOcean Spaces, and Backblaze B2 are reasonable
alternatives only when a named constraint blocks R2. An existing AWS billing
relationship counts, and so does a geographic gap in R2's PoP coverage. Self-hosted MinIO / Garage /
...
- **Preview deploys per PR, full-stack.** Frontend, API, **and an ephemeral
  ...
  The goal: open a PR, the reviewer clicks a link, and the actual feature runs
  on the actual stack. No checkout, no `pnpm install`, no "works on my machine."
  Frontend-only previews are the fallback when full-stack is genuinely hard to
  set up. Once you have full-stack, you don't go back.
...
ordering, and back-pressure. They add an entirely different observability story
(per §12). They add a long-lived stateful object that complicates §3's
single-instance default, because sticky sessions become a thing once a second
instance earns its keep.
...
1. **Latency-critical.** The event must reach the user well inside one polling
   interval. Collaborative editing, a live order book, real-time chat where the
...
3. **Server-to-server webhooks** for an external system calling *into* your app
   This is the inverse of polling an upstream API. A webhook lets the upstream tell you
...
- **WebSockets** only when you need bidirectional traffic or sub-100 ms latency
  Examples: collaborative editing, multiplayer, voice signaling.
