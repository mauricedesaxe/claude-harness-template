...
Use `/code-review` where possible, then use /diagnosing-bugs to review the work.
...
The throwaway route under `/code-review/<name>` mounts the same switcher.
...
#!/usr/bin/env bash
# Generated by the /codebase-design skill.
# Hand the captured values to `/handoff`. Stages live under /codebase-design/stages.
EOF

apply_prefix_to_references "$fixture"

assert_contains "a backticked reference is prefixed" '`/matt-code-review`'
assert_contains "a bare reference is prefixed" 'use /matt-diagnosing-bugs to review'
...
assert_contains "a route starting with a skill name is left alone" '`/code-review/<name>`'

assert_file_contains "a bare reference in a shell file is prefixed" template.sh \
  'Generated by the /matt-codebase-design skill'
assert_file_contains "a backticked reference in a shell file is prefixed" template.sh \
  '`/matt-handoff`'
assert_file_contains "a path in a shell file is left alone" template.sh '/codebase-design/stages'
